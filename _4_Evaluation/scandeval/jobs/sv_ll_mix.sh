...
#SBATCH --job-name=sv_ll_mix       # Job name
#SBATCH --output=sv_ll_mix.out  # Name of output file
...
# Run script
echo "Scandeval using meelu/DA-MIXED-LLAMA3.2"

# Run scandeval while disabling VLLM to avoid issues with custom tokenizers
USE_VLLM=0 scandeval --model meelu/DA-MIXED-LLAMA3.2 --task linguistic-acceptability --task summarization --language da --no-use-flash-attention --device cuda --force --use-token --token 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 --clear-model-cache --verbose --debug
