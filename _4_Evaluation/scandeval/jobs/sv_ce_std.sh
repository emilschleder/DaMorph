...
#SBATCH --job-name=sv_ce_std       # Job name
#SBATCH --output=sv_ce_std.out  # Name of output file
...
# Run script
echo "Scandeval using meelu/STD-BPE-CEREBRAS"

# Run scandeval while disabling VLLM to avoid issues with custom tokenizers
USE_VLLM=0 scandeval --model meelu/STD-BPE-CEREBRAS --task linguistic-acceptability --task summarization --language da --no-use-flash-attention --device cuda --force --use-token --token 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 --clear-model-cache --verbose --debug
