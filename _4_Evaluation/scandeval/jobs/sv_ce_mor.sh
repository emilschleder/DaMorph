...
#SBATCH --job-name=sv_ce_mor       # Job name
#SBATCH --output=sv_ce_mor.out  # Name of output file
...
# Run script
echo "Scandeval using meelu/DA-MORPH-CEREBRAS"

# Run scandeval while disabling VLLM to avoid issues with custom tokenizers
USE_VLLM=0 scandeval --model meelu/DA-MORPH-CEREBRAS --task linguistic-acceptability --task summarization --language da --no-use-flash-attention --device cuda --force --use-token --token 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 --clear-model-cache --verbose --debug
